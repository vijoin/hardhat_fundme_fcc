...
describe("FundMe", function () {
    ...
    it("only owner can withdraw funds", async function () {
        ...
        await expect(fundme.connect(funder1).withdraw()).to.be.revertedWith("Ownable: caller is not the owner");
    })
    ...
    it("registry of funding accounts and amounts must be reset after withdraw", async function () {
        ...
        expect(await fundme.addressToAmount(funder1.address)).to.equal(ethZeroAmount);
        expect(await fundme.addressToAmount(funder2.address)).to.equal(ethZeroAmount);
    })
